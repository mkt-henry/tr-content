 ...
 * 모든 ARIA 데모가 useLang()으로 같은 값을 구독한다. 기본값은 한국어.
 */
...
/** 현재 ARIA 언어 — 컴포넌트에서 구독 */
export function useLang(): Lang {
  return useShellStore((s) => (s.projectLang['aria'] as Lang | undefined) ?? DEFAULT_LANG);
}

/** 비리액티브 조회 — 시나리오/스토어 등 훅 밖에서 사용 */
export function getLang(): Lang {
  return (useShellStore.getState().projectLang['aria'] as Lang | undefined) ?? DEFAULT_LANG;
}
