    ...
    <div className="flex h-full bg-[#101418] text-zinc-200">
      ...
      <aside className="flex w-60 shrink-0 flex-col border-r border-white/[0.06] bg-[#0c1014]">
        <div className="flex items-center gap-2 px-4 py-4">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[12.5px] font-normal text-zinc-500">by AlphaLenz</span>
          </div>
        </div>
        <button className="mx-3 flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2.5 text-[15.5px] text-zinc-300 hover:bg-white/[0.07]">
          <Plus className="h-4 w-4" /> {pick(STR.newChat, lang)}
        </button>
        <div className="mx-3 mt-3 flex items-center gap-2 rounded-lg bg-white/[0.03] px-3 py-2 text-[15.5px] text-zinc-600">
          <Search className="h-3.5 w-3.5" /> {pick(STR.searchChats, lang)}
        </div>
        <nav className="mt-4 flex-1 px-3">
          <p className="mb-2 px-1 text-[13px] font-medium uppercase tracking-wider text-zinc-600">
            {pick(STR.recentChats, lang)}
          </p>
          {/* 진행 중인 대화 — 첫 질문을 활성 항목으로 노출 */}
          {currentTitle && (
            <button className="flex w-full items-center gap-2 rounded-lg bg-white/[0.06] px-3 py-2 text-left text-[15.5px] text-zinc-200">
          ...
          {pick(HISTORY, lang).map((h, i) => (
            <button
              key={i}
              className={cn(
                'flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-[15.5px]',
      ...
      <main className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="border-t border-white/[0.06] p-4">
            ...
            {pipeline && (
              <div className="mb-2 flex items-center gap-2">
                <span
                  ...
                  className="inline-flex items-center gap-1.5 rounded-lg border border-teal-500/30 bg-teal-500/10 py-1 pl-2 pr-1 text-[14.5px] text-teal-200"
                >
          ...
          <p className="mx-auto mt-2 max-w-2xl text-center text-[13px] text-zinc-600">{pick(STR.footerNote, lang)}</p>
        </div>
      </main>
    </div>
  );
