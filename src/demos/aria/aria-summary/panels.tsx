...
import { CLAUSES, SUMMARY_ITEMS, DOC_NAME, STR } from './data';
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex shrink-0 items-center gap-2 border-b border-white/[0.06] px-4 py-2.5">
        ...
        <span className="text-[12px] font-medium text-zinc-200">{pick(STR.summaryTitle, lang)}</span>
        <span className="rounded bg-brass-500/15 px-1.5 py-0.5 font-mono text-[9px] uppercase text-brass-300">{lang}</span>
        <AnimatePresence>
          {s.phase === 'done' && (
            <motion.span
      ...
      <div className={cn('demo-scroll demo-scroll-follow min-h-0 flex-1 overflow-y-auto p-4', compact && 'p-3')}>
        {s.phase === 'idle' ? (
          <div className="flex h-full flex-col items-center justify-center gap-4 text-center">
            ...
            <div>
              <p className="text-[14px] font-medium text-zinc-300">{pick(STR.emptyTitle, lang)}</p>
              <p className="mt-1 text-[11.5px] text-zinc-500">{pick(STR.emptySubtitle, lang)}</p>
            </div>
            <button
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2">
                    <p className="text-[11px] font-medium text-zinc-500">{pick(item.label, lang)}</p>
