...
import { CANDIDATES, RISK_SUMMARY, STR } from './data';
...
export function RiskCard({ compact }: { compact?: boolean }) {
  ...
  return (
    <div className="rounded-xl border border-white/[0.08] bg-white/[0.03] p-4">
      <p className="text-[10.5px] font-medium uppercase tracking-wider text-zinc-500">{pick(STR.riskHeader, lang)}</p>
      ...
      <div className={cn('mt-3 grid gap-2', compact ? 'grid-cols-2' : 'grid-cols-1')}>
        {RISK_SUMMARY.items.map((it) => (
          <div key={it.label[lang]} className="flex items-baseline justify-between gap-2 rounded-lg bg-white/[0.03] px-3 py-2">
            <span className="text-[10.5px] text-zinc-500">{pick(it.label, lang)}</span>
            <span className="font-mono text-[11.5px] text-zinc-200">{pick(it.value, lang)}</span>
          </div>
        ))}
      </div>
      <button
      ...
      >
        ...
        {phase === 'idle'
          ? pick(STR.analyzeIdle, lang)
          : phase === 'scoring'
            ? pick(STR.analyzeScoring, lang)
            : pick(STR.analyzeDone, lang)}
      </button>
    </div>
  );
}
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="w-28 shrink-0">
                <div className="flex items-baseline justify-between">
                  <span className="text-[9.5px] text-zinc-500">{pick(STR.fitLabel, lang)}</span>
              ...
              {m.ranked && isTop && !compact && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-3">
                  ...
                  >
                    ...
                    {selected ? pick(STR.selectBusy, lang) : pick(STR.selectFull, lang)}
                  </button>
                </motion.div>
              )}
              {m.ranked && isTop && compact && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-3">
                  ...
                  >
                    <Mail className="h-3.5 w-3.5" /> {pick(STR.selectCompact, lang)}
                  </button>
                </motion.div>
              )}
            </AnimatePresence>
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="demo-scroll demo-scroll-follow min-h-0 flex-1 overflow-y-auto p-4">
        <div className="rounded-lg bg-white/[0.03] px-3.5 py-2.5">
          <p className="text-[9.5px] uppercase tracking-wider text-zinc-600">{pick(STR.toLabel, lang)}</p>
