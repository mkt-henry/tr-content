...
export const evidenceScenario: Scenario = {
  ...
  steps: [
    { kind: 'wait', ms: 900 },
    { kind: 'type', target: 'chat-input', text: SUGGESTED[0], cps: 14, set: (v) => st().setInput(v) },
    { kind: 'wait', ms: 400 },
...
export const naturalScenario: Scenario = {
  ...
  steps: [
    ...
    { kind: 'wait', ms: 300 },
    { kind: 'click', target: 'suggest-2', run: () => st().send(SUGGESTED[2]) },
    { kind: 'wait', ms: 8500 },
    { kind: 'type', target: 'chat-input', text: SUGGESTED[1], cps: 15, set: (v) => st().setInput(v) },
    { kind: 'wait', ms: 350 },
    { kind: 'click', target: 'chat-send', run: () => st().send() },
    { kind: 'wait', ms: 9000 },
    { kind: 'wait', ms: 1200 },
