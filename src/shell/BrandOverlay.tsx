 ...
 * portrait=true(모바일)면 스테이지 높이를 꽉 채우는 세로 패널로 감싸 광고/릴스 비율로 렌더한다.
 * portraitAspect로 패널 비율을 지정한다 — '3/4'(모바일 광고 규격) 또는 '9/16'(릴스/숏츠, 기본).
 * 패널이 스테이지(=녹화 크롭) 높이를 가득 채우므로 배경이 프레임 전체를 덮는다(레터박스 없음).
...
export function BrandOverlay({
  ...
  return (
    ...
    >
      {portrait ? (
        // 세로 패널을 스테이지(=녹화 크롭) 높이에 꽉 채워 풀블리드로 렌더.
        // aspectRatio가 녹화 크롭 비율과 일치해 사방 레터박스 없이 배경이 프레임을 덮는다.
        <div
          className="relative h-full overflow-hidden"
          style={{ aspectRatio: portraitAspect === '3/4' ? '3 / 4' : '9 / 16' }}
        >
          <Phase portrait />
        </div>
      ) : (
